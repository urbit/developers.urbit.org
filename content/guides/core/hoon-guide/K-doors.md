...
- implementing state machines (the [subject-oriented programming module](/guides/core/hoon-guide/O-subject))
...
The standard library provides [currying functionality](/guides/core/hoon-guide/Q-func) outside of the context of doors.
...
Here the type of `b` is inferred to be `@` based on the example value `7`, similar to how we've seen casting done by example.  You will learn more about how types are inferred in the [next module](/guides/core/hoon-guide/L-struct).
...
`unit`s are common enough that they have their own syntax and set of operational functions.  We'll look at them more in [the next module](/guides/core/hoon-guide/K-doors).
