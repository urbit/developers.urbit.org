...
import languageTree from "../../cache/language.json";
...
export default function GuidePage({
  ...
  if (!params.slug) {
    return (
      <BasicPage
        wide
        post={{ title: data.title, slug: "/language" }}
        markdown={markdown}
        search={search}
      />
    );
  ...
  return (
    <>
      <Head>
        <title>{data.title} • Language • developers.urbit.org</title>
        {Meta(data)}
      </Head>
      <div className="flex h-screen min-h-screen w-screen sidebar">
        <Sidebar search={search}>
          <RenderTree root="/language" posts={posts.children} />
        </Sidebar>
        <ContentArea
        ...
        >
          ...
          <div className="flex justify-between mt-16">
            {previousPost === null ? (
              ...
              <Pagination
                ...
                post={previousPost}
                className=""
                section={join("language", params.slug?.slice(0, -1).join("/"))}
              />
            )}
            {nextPost === null ? (
              <div className={""} />
          ...
          <div className="flex justify-between items-center mt-16">
            <a
              ...
              target="_blank"
              href={`https://github.com/urbit/developers.urbit.org/blob/main/content/language/${params.slug?.join("/") || "_index"
                }.md`}
            >
...
const breadcrumbs = (posts, paths) => {
  const results = [<Link href="/language">Language</Link>];
  let thisLink = "/language";
...
export async function getStaticProps({ params }) {
  let posts = languageTree;

  const { data, content } = getPage(
    join(process.cwd(), "content/language", params.slug?.join("/") || "/"), true
  );

  const previousPost =
    getPreviousPost(
      params.slug?.slice(-1).join("") || "language",
      ["title", "slug", "weight"],
      join("language", params.slug?.slice(0, -1).join("/") || "/"),
      "weight"
    ) || null;
