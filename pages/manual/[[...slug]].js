...
export default function GuidePage({
  ...
  if (!params.slug) {
    return (
      <BasicPage
        wide
        post={{ title: data.title, slug: "/manual" }}
        markdown={markdown}
        search={search}
      />
    );
  ...
  return (
    <>
      <Head>
        <title>{data.title} • Manual • developers.urbit.org</title>
        {Meta(data)}
      </Head>
        ...
        >
          ...
          <div className="flex justify-between mt-16">
            {previousPost === null ? (
              ...
              <Pagination
                ...
                post={previousPost}
                className=""
                section={join("manual", params.slug?.slice(0, -1).join("/"))}
              />
            )}
            {nextPost === null ? (
              <div className={""} />
          ...
          <div className="flex justify-between items-center mt-16">
            <a
              ...
              target="_blank"
              href={`https://github.com/urbit/developers.urbit.org/blob/main/content/manual/${params.slug?.join("/") || "_index"
                }.md`}
            >
...
const breadcrumbs = (posts, paths) => {
  const results = [<Link href="/manual">Manual</Link>];
  let thisLink = "/manual";
...
export async function getStaticProps({ params }) {
  let posts = manualTree;

  const { data, content } = getPage(
    join(process.cwd(), "content/manual", params.slug?.join("/") || "/"), true
  );

  const previousPost =
    getPreviousPost(
      params.slug?.slice(-1).join("") || "manual",
      ["title", "slug", "weight"],
      join("manual", params.slug?.slice(0, -1).join("/") || "/"),
      "weight"
    ) || null;
