...
import systemTree from "../../cache/system.json";
...
export default function GuidePage({
  ...
  if (!params.slug) {
    return (
      <BasicPage
        wide
        post={{ title: data.title, slug: "/system" }}
        markdown={markdown}
        search={search}
      />
    );
  ...
  return (
    <>
      <Head>
        <title>{data.title} • System • developers.urbit.org</title>
        {Meta(data)}
      </Head>
      <div className="flex h-screen min-h-screen w-screen sidebar">
        <Sidebar search={search}>
          <RenderTree root="/system" posts={posts.children} />
        </Sidebar>
        <ContentArea
        ...
        >
          ...
          <div className="flex justify-between mt-16">
            {previousPost === null ? (
              ...
              <Pagination
                ...
                post={previousPost}
                className=""
                section={join("system", params.slug?.slice(0, -1).join("/"))}
              />
            )}
            {nextPost === null ? (
              <div className={""} />
          ...
          <div className="flex justify-between items-center mt-16">
            <a
              ...
              target="_blank"
              href={`https://github.com/urbit/developers.urbit.org/blob/main/content/system/${params.slug?.join("/") || "_index"
                }.md`}
            >
...
const breadcrumbs = (posts, paths) => {
  const results = [<Link href="/system">System</Link>];
  let thisLink = "/system";
...
export async function getStaticProps({ params }) {
  let posts = systemTree;

  const { data, content } = getPage(
    join(process.cwd(), "content/system", params.slug?.join("/") || "/"), true
  );

  const previousPost =
    getPreviousPost(
      params.slug?.slice(-1).join("") || "system",
      ["title", "slug", "weight"],
      join("system", params.slug?.slice(0, -1).join("/") || "/"),
      "weight"
    ) || null;
